\section{The English Copula, Raising Verbs, and Small Clauses}
...
sentences ({\ex{1}}) and ({\ex{2}}).  Because the complement phrases predicate over
...
infinitive, as seen in the sentences in ({\ex{1}}).  They cannot, however, invert
or contract like other auxiliaries ({\ex{2}}), and they appear to the right of
adverbs ({\ex{3}}).
...
\enumsentence{$*$Seems Carl to be a jerk?\\
...
\enumsentence{$*$I prefer [Carl a jerk].\\
		??I prefer [Carl upset].\\
		I prefer [Carl in a foul mood].}


\subsection{Various Analyses}
...
\enumsentence{Carl became a jerk.}
\enumsentence{Carl became upset.}
\enumsentence{$*$Carl became in a foul mood.}
...
\enumsentence{$*$I consider the culprit John.}
...
({\ex{1}})~-~({\ex{3}}) for NPs, APs, and PPs.  The XTAG trees corresponding to
...
\enumsentence{$*$She prefers Carl a jerk.\\
		She prefers Carl in a foul mood.\\
		She prefers that Carl is a jerk.}

\enumsentence{$*$She feels Carl a jerk.\\
...
\subsection{Non-predicative {\it BE}}
...
\enumsentence{$*$My teacher seems Mrs. Wayman.}
\enumsentence{$*$Doug appears the man with the glasses.}

\enumsentence{$*$I consider [my teacher Mrs. Wayman].}
\enumsentence{$*$I prefer [Doug the man with the glasses].}
...
the sentence with the exchanged NPs ({\ex{3}}) is ungrammatical.

\enumsentence{The man with the glasses is Doug.}
\enumsentence{Doug is a programmer.}
\enumsentence{$*$A programmer is Doug.}

The non-predicative {\it be} in ({\ex{-8}}) and ({\ex{-7}}), also called {\bf
...
{\it be}.  The sentences in ({\ex{1}}) contain predicative {\it be} as
...
grammaticality of the sentence produced by the equivalent switch in ({\ex{2}})
...
$*$To provide a non-solution stipulative solution is the challenger.\\
...
To provide a non-stipulative solution is the challenge.\\
		$*$What$_{i}$ is the challenge to provide $t_{i}$?}
