...
Table~\ref{det-values}.  No {\bf $<$agr$>$} is specified however, since the
number and person of the genitive will depend on its particular form (e.g.\
{\it my} vs {\it their}).  Genitives are not required to agree with either the
...
There are two things to note in the sentence~(\ex{0}).  One is that in embedded
